import { createServiceClient, processRejectionEmail } from "../_shared/rejection-email.ts";
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .select("entry_id, rejected_at, status, attempt_count")
    ...
    for (const row of data || []) {
      try {
        const result = await processRejectionEmail(supabase, row.entry_id, row.rejected_at);
